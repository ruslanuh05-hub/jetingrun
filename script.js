    ...
    fetch(url)
        ...
        .catch((err) => {
            ...
            if (!msg) {
                ...
                else msg = 'Сервер бота недоступен. Если API работает по http://localhost:3000 — откройте мини-приложение с этого ПК по адресу http://localhost:8080 (в папке mini_app выполните: python -m http.server 8080). С jetstoreapp.ru запросы идут на jetstoreapp.ru, а не на localhost. См. SETUP_API.md.';
            }
            if (inputId === 'starsRecipient') {
